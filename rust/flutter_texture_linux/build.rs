...
lazy_static! {
    static ref GIT_ROOT: PathBuf = std::env::current_dir().unwrap().join("../..").canonicalize().unwrap();
...
fn main() {
    ...
    run_cmd("flutter", &[ "build", "linux", "--release"], Some(&EXAMPLE_DIR));
    ...
    println!("cargo:rustc-link-search={}", root_linux_path.join("build").to_str().unwrap());
